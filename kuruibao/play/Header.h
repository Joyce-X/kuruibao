...
#define AJ_FileCache_Firmware @"AJ_FileCache_Firmware"
/**
 *  条件编译选项...
 */
...
/*
 ...
 #ifdef DEBUG
 #   define NSLogD(fmt, ...) NSLogD((@"%s [Line %d] " fmt), __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__);
 #else
 #   define NSLogD(...)
 #endif
 ...
 #ifdef ITTDEBUG
 #define ITTDPRINT(xx, ...)  NSLogD(@"%s(%d): " xx, __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__)
 #else
