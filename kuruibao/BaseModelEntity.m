...
- (id)init
{
    ...
    if (self) {
        ...
        self.requestHelper = [[RequestHelper alloc] initWithHostName:@"192.168.63.9" customHeaderFields:@{@"x-client-identifier" : @"iOS"}];
...
- (void)removeLastUpdateWarePatchFile
{
    ...
    NSArray *pathArray = [fm contentsOfDirectoryAtPath:[[XYSandbox libCachePath] stringByAppendingPathComponent:AJ_FileCache_Firmware]
                                                 error:nil];
    ...
    if (pathArray)
    {
        ...
        {
            [self removeFileName:path withFolderPath:[[XYSandbox libCachePath]
                                                      stringByAppendingPathComponent:AJ_FileCache_Firmware]];
        }
    }
}
